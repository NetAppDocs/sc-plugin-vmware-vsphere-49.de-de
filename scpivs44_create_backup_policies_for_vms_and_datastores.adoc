...
. Geben Sie in den Feldern *Replikation* den Replikationstyp für sekundären Speicher an, wie in der folgenden Tabelle dargestellt:
+
|===
...
| Schließen Sie Datastores mit ein
Unabhängige Festplatten | Aktivieren Sie dieses Kontrollkästchen, um alle Datenspeicher mit unabhängigen Festplatten, die temporäre Daten enthalten, in das Backup einzubeziehen. 
...
NOTE: Prescripts und Postscripts müssen auf der VM der virtuellen Appliance liegen.
Um mehrere Skripte einzugeben, drücken Sie nach jedem Skriptpfad zur Liste *Enter*
Jedes Skript in einer eigenen Zeile. Das Zeichen „;“ ist nicht zulässig.
