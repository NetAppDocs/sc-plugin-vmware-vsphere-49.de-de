...
| 1 | `/4.1/storage-system` | `getSvmAll` Ruft die Liste aller verfügbaren Storage VMs ab.
Beachten Sie den *Namen* der Speicher-VM, die Sie ändern möchten. 


| 2 | `/4.1/storage-system` | `Modify Storage System` Ändert die angegebene Storage-VM.
Übergeben Sie den *Name* aus Schritt 1 zusätzlich zu allen anderen erforderlichen Attributen. 
|===